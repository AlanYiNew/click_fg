...
/*
 * =c
 * Camkes_IPFragmenter(MTU, [I<keywords> HONOR_DF, VERBOSE])
 * =s ip
 ...
 * don't-fragment bit (DF) isn't set, Camkes_IPFragmenter splits the packet into
 ...
 * Camkes_IPFragmenter, since any MAC header is not copied to second and subsequent
 * fragments.
 *
 ...
 * =item HONOR_DF
 *
 * Boolean. If HONOR_DF is false, Camkes_IPFragmenter will ignore the don't-fragment
 * (DF) bit and fragment every packet larger than MTU. Default is true.
 *
 * =item VERBOSE
 *
 * Boolean.  If true, Camkes_IPFragmenter will print a message every time it sees a
 ...
 *
 * =e
 *   ... -> fr::Camkes_IPFragmenter(1024) -> Queue(20) -> ...
 *   fr[1] -> ICMPError(18.26.4.24, 3, 4) -> ...
 *
 * =a ICMPError, CheckLength
 */

class Camkes_IPFragmenter : public Element { public:

  Camkes_IPFragmenter() CLICK_COLD;
  ~Camkes_IPFragmenter() CLICK_COLD;
  Camkes_IPFragmenter(message_t*);

  const char *class_name() const		{ return "Camkes_IPFragmenter"; }
