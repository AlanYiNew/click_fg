 ...
 * =c
 *
 * Camkes_ARPResponder(IP/MASK [IP/MASK...] ETH, IP2/MASK2 ETH2, ...)
 *
 * =s arp
 *
 ...
 *
 * Camkes_ARPResponder sets the device and VLAN TCI annotations on generated ARP
 * responses to the corresponding annotations from the queries.
 *
 * =n
 *
 * AddressInfo elements can simplify the arguments to Camkes_ARPResponder. In
 * particular, if C<NAME> is shorthand for both an IP network address (or IP
 * address) C<IP> and an Ethernet address C<ETH>, then C<Camkes_ARPResponder(NAME)> is
 * equivalent to C<Camkes_ARPResponder(IP ETH)>. If C<NAME> is short for both an IP
 * address and an IP network address, then Camkes_ARPResponder will prefer the IP
 * address. (You can say C<NAME:ipnet> to use the IP network address.)
 *
 * =h table r
 *
 * Return the Camkes_ARPResponder's current table, with one IP network entry per
 * line.
 *
 ...
 *   ar :: Camkes_ARPResponder(18.26.4.24 18.26.7.0/24 00-00-C0-AE-67-EF);
 *   c[0] -> ar;
 *   ar -> ToDevice(eth0);
 *
 * =a
...
class Camkes_ARPResponder : public Element { public:

    Camkes_ARPResponder() CLICK_COLD;
    ~Camkes_ARPResponder() CLICK_COLD;

    const char *class_name() const		{ return "Camkes_ARPResponder"; }
